...
<?php include_once("header.php"); ?> 
        ...
        <section class="wall">
                <?php 
                    ...
                    $sql = "SELECT id,title,image,content,`date`,type FROM(
                    ...
                    $total_query = "SELECT count(id) as total FROM(
                    SELECT id FROM articles as a WHERE `deleted` = 0
                    UNION ALL
                    SELECT id FROM experiences as e WHERE `deleted` = 0
                    UNION ALL
                    SELECT id FROM questions as q WHERE `deleted` = 0) as w
                    ";
                    $total = get_query_total($total_query);
 ...
 <div class="text-box ask-box">
            ...
            <div class="row">
                ...
                <?php 
                    ...
                    $date = $row['date'];
                    $type = $row['type'];//article/expereince/question
...
<a href="article.php?id=<?php echo $id;?>&type=<?php echo $type; ?>" >قراءة المزيد
        ...
        //get data when click load more
            ...
            $(".btn-load-more").click(function () {
                $.getJSON(
                    'Controller/get_wall_json.php', //الرابط اللي طالبينه
                    {'init': init }, //sent data
                    function (data) {//دالة النجاح
                        
                        for(var i=0;i<data.details.length;i++){
                            var id = data.details[i].id;
                            var type = data.details[i].type;
