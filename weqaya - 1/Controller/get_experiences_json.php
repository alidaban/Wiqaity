...
$type = "experience";//article, experience, question
$from = $_GET['init'];
$limit =  5;

$result = $conn->query("SELECT `id`, `title`, `story`, `date`, `image`, `source`, `writer` FROM `experiences` WHERE `deleted` = 0 LIMIT $limit OFFSET $from");
//to get the total
...
if ($result->num_rows > 0) 
{
    ...
    while ($row = $result->fetch_assoc())
    {
        ...
        //short content
        $article["content"] = string_limit_words(removeTAGS(stripslashes(trim($row["story"]))),50);
