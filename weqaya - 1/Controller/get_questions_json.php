...
$result = $conn->query("SELECT `id`, `question`, `answer`, `date`, `image`, `writer` FROM `questions` WHERE `deleted` = 0 LIMIT $limit OFFSET $from");
//to get the total
...
if ($result->num_rows > 0) 
{
    ...
    while ($row = $result->fetch_assoc())
    {
        ...
        $article["title"] = convert_to_UTF8(Clear(strip_tags($row["question"])));

        //short content
        $article["content"] = string_limit_words(removeTAGS(stripslashes(trim($row["answer"]))),50);
